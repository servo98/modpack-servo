# Gacha Rates - Modpack Servo

> Consistente con docs/mechanics/gacha.md (4 maquinas por tier)

## Token Economy

### Sources (150 tokens/capitulo)
| Fuente | Tokens/cap | % |
|--------|-----------|---|
| Quests FTB (~15/cap) | 35 | 23% |
| Boss capitulo | 20 | 13% |
| Delivery capitulo | 15 | 10% |
| Recetas nuevas (~15/cap) | 15 | 10% |
| Champions overworld | 25 | 17% |
| Dungeon runs (~3/cap, promedio all tiers) | 40 | 27% |
| **Total** | **150** | 100% |

### Sinks (150 tokens/capitulo)
| Gasto | Tokens | % |
|-------|--------|---|
| Gacha artefactos (~10 pulls) | 100-130 | ~83% |
| Gacha muebles (~4 pulls) | 20 | ~13% |
| **Total** | **~150** | ~100% |

> Llaves de dungeon se craftean con materiales, no consumen tokens. Ver [dungeons.md](../mechanics/dungeons.md).

> Nota: Los sinks varian segun el jugador. Algunos priorizaran muebles, otros gacha avanzada. Los numeros son promedios.
> **PepeMart NO consume tokens** — usa materiales. Ver [catalogo](pepemart-catalog.md).

### Costs
| Accion | Costo |
|--------|-------|
| 1 pull Basica (Verde) | 10 tokens |
| 1 pull Avanzada (Azul) | 10 tokens |
| 1 pull Superior (Purpura) | 15 tokens |
| 1 pull Muebles (Rosa) | 5 tokens |

## Rates por maquina

### Basica (Verde) — Ch1+
| Rareza | Prob | Contenido clave |
|--------|------|-----------------|
| Comun | 55% | Materiales vanilla, ingredientes, runas small |
| Poco Comun | 25% | Gemas Citrine/Jade, libros I-II, preview crops |
| Raro | 12% | Accesorio T1, Copper/Iron Ring |
| Epico | 5% | Accesorio T2, Gold/Gem Ring |
| Legendario | 3% | Arma RPG T1, Accesorio T2 especial, Llave Basica |

### Avanzada (Azul) — Ch3+
| Rareza | Prob | Contenido clave |
|--------|------|-----------------|
| Comun | 50% | Diamond, Amethyst, Blaze Rod, runas medium |
| Poco Comun | 25% | RPG armor T1, Ruby/Sapphire jewelry, libros II-III |
| Raro | 15% | Accesorio T2, RPG armor clase especifica |
| Epico | 7% | Accesorio T3, Unique Jewelry basica (8/24) |
| Legendario | 3% | Arma RPG T2, Accesorio T3 especial, Unique Jewelry media |

### Superior (Purpura) — Ch5+
| Rareza | Prob | Contenido clave |
|--------|------|-----------------|
| Comun | 45% | Netherite Scrap, Diamond, Ghast Tear, runas large |
| Poco Comun | 25% | RPG armor T2, Netherite jewelry, enchants magicos |
| Raro | 17% | Accesorio T3, Unique Jewelry media (16/24) |
| Epico | 9% | Accesorio T4, Unique Jewelry completa (24/24), Llave Avanzada |
| Legendario | 4% | Accesorio T4 con bonus doble, Unique Jewelry legendaria, arma T3 |

### Muebles (Rosa) — Ch2+
| Rareza | Prob | Contenido clave |
|--------|------|-----------------|
| Comun | 60% | Muebles basicos + plushies comunes + Create Deco basico |
| Poco Comun | 25% | Muebles decorativos + plushies especiales + Create Deco medio |
| Raro | 10% | Muebles especiales + funcionales (fridge/stove/sink) — **SIN plushies** |
| Epico | 4% | Sets tematicos + plushies epicos (8 items) |
| Legendario | 1% | Exclusivos + plushies legendarios (5 items) + Heart of Gold |

> Items exactos por rareza: ver [Catalogo Completo](pepemart-catalog.md) (fuente de verdad)

## Pity System

- +2% a Epico por cada pull sin Epico+ (por maquina, por jugador)
- Hard pity a 50 pulls: garantiza Legendario
- Se reinicia al obtener Epico+
- Pity NO se comparte entre maquinas

### Pity math
| Pull # | Prob Epico+ efectiva (Basica) |
|--------|-------------------------------|
| 1-10 | 8% (5+3 base) |
| 11 | 10% |
| 20 | 28% |
| 30 | 48% |
| 40 | 68% |
| 50 | 100% (hard pity) |

> En promedio, un jugador obtiene su primer Epico+ a los ~12-15 pulls por maquina.

## Lifetime Stats (estimado total del modpack)

### Pulls estimados por maquina (8 capitulos)
| Maquina | Capitulos activos | Pulls estimados | Costo total |
|---------|-------------------|-----------------|-------------|
| Basica | Ch1-Ch8 (8) | ~60 | 600 tokens |
| Avanzada | Ch3-Ch8 (6) | ~36 | 360 tokens |
| Superior | Ch5-Ch8 (4) | ~16 | 240 tokens |
| Muebles | Ch2-Ch8 (7) | ~35 | 175 tokens |
| **Total** | | **~147** | **1,375 tokens** |

> 150 tokens/cap x 8 caps = 1,200 tokens. El exceso (~175) viene de ahorrar en dungeons/llaves o farmear champions extra.

### Items esperados por rareza (Basica, 60 pulls)
| Rareza | Esperados |
|--------|-----------|
| Comun | ~33 |
| Poco Comun | ~15 |
| Raro | ~7 |
| Epico | ~3 |
| Legendario | ~2 |

### Items esperados por rareza (Avanzada, 36 pulls)
| Rareza | Esperados |
|--------|-----------|
| Comun | ~18 |
| Poco Comun | ~9 |
| Raro | ~5 |
| Epico | ~3 |
| Legendario | ~1 |

### Items esperados por rareza (Superior, 16 pulls)
| Rareza | Esperados |
|--------|-----------|
| Comun | ~7 |
| Poco Comun | ~4 |
| Raro | ~3 |
| Epico | ~1-2 |
| Legendario | ~1 |

### Accesorios custom de gacha (lifetime)
| Fuente | T1 | T2 | T3 | T4 | T5 | Total |
|--------|----|----|----|----|-----|-------|
| Basica (60 pulls) | ~7 | ~3 | — | — | — | ~10 |
| Avanzada (36 pulls) | — | ~5 | ~3 | — | — | ~8 |
| Superior (16 pulls) | — | — | ~3 | ~1-2 | — | ~4-5 |
| **Total gacha** | **~7** | **~8** | **~6** | **~1-2** | **—** | **~22-23** |

> T5 Maestro (4 items, 1 por slot) es exclusivo de Boss Ch7 y Ch8. NO aparece en gacha.

### Unique Jewelry de gacha (lifetime)
| Fuente | Cantidad esperada |
|--------|-------------------|
| Avanzada Epico (36 pulls × 7%) | ~2-3 del pool basico (8 items) |
| Avanzada Legendario (36 pulls × 3%) | ~1 del pool medio |
| Superior Raro (16 pulls × 17%) | ~2-3 del pool medio (16 items) |
| Superior Epico (16 pulls × 9%) | ~1-2 del pool completo (24 items) |
| Superior Legendario (16 pulls × 4%) | ~1 del pool legendario (8 items) |
| **Total gacha** | **~7-10 unique jewelry** |

> Complementado por dungeon drops (~5-8 unique jewelry). Total esperado: ~12-18 de 24 unique jewelry coleccionadas.
